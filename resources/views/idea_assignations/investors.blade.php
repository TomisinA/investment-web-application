...
<body style="display:flex; min-height:100vh;">
    ...
    <div class="Wrapper" style="width: 100%; display:flex;">
        <div class="sidebar" id="sidebar">
          ...
          <ul>
            <li><a class="mt-5" href="{{ route('show-ideas') }}">Ideas</a></li>
            <li><a class="mt-5" href="{{ route('investors') }}">Investors</a></li>
            <li><a class="mt-5" href="{{ route('assignations') }}">Assignations</a></li>
            <li>
        ...
        <div class="main-view" style="width:100%; display:flex; flex-direction:column; justify-content:space-between">
          ...
          <main style="padding:32px;">
            <form action="{{ route('assign-idea') }}" method="POST" >
              @csrf

              @unless(count($investors) == 0)
                      ...
                      <table class="table table-striped table-bordered mt-5" id="dashboard">
                          ...
                          <tbody>
                              ...
                              @foreach ($investors as $investor)
                                <tr>
                                  <td><input type="radio" name="investor_id" value="{{$investor->id}}"></td>
                                  <td>{{ $investor->name }}</td>

                                  <td>
                                    <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal{{$investor->id}}" style="border:none; background:transparent">
                                      <img src="images/assign_idea.png" alt="" height="24px" width="40px">
                                    </button>
                                    <div class="modal fade" id="exampleModal{{$investor->id}}" tabindex="-1" aria-labelledby="exampleModalLabel{{$investor->id}}" aria-hidden="true">
                                      <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                          <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="exampleModalLabel{{$investor->id}}">Modal title</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                          </div>
                                          <div class="modal-body">
                                            <div class="" id="datatable">
                                              <table class="table table-striped mt-5" id="modaltable">
                                                <thead>
                                                  <tr>
                                                    <th>Select</th>
                                                ...
                                                <tbody>
                                                  @foreach($ideas as $idea)
                                                    <tr>
                                                      <td><input type="checkbox" name="idea_id[]" value="{{$idea->id}}"></td>
                                                      <td>{{$idea->title}}</td>
                                          ...
                                          <div style="margin-top:40px; display:flex; flex-direction:column; align-items:center;" class="mb-5">
                                            <button type="submit" id="submit-btn" class="mt-2 button-assigned-idea">Submit</button>
                                          </div>
                                          </div>
                                        </div>
                                      </div>
    ...
    <script>
      ...
      const ideaCheckboxes = document.querySelectorAll('input[name="idea_id[]"]');

      // Disable submit button by default
      submitBtn.disabled = true;

      // Add event listener to idea checkboxes
      ideaCheckboxes.forEach((checkbox) => {
        checkbox.addEventListener('change', () => {
          // Enable submit button if any idea checkbox is checked
          submitBtn.disabled = !Array.from(ideaCheckboxes).some((c) => c.checked);
        });
      });
    </script>
